...
export default function TowerScoutShell() {
    ...
    const projects: Project[] = useMemo(() => {
        ...
        return Object.values(groups).map(group => {
            ...
            group.forEach(p => {
                if (p.serverCreatedAt) {
                    const ts = new Date(p.serverCreatedAt).getTime();
                    if (!isNaN(ts)) dateObjs.push(ts);
                }
            });
            ...
            return {
                ...
                timeline: mergedSignals.map((s: any) => {
                    const d = s.article_date ? new Date(s.article_date) : new Date(s.created_at);
